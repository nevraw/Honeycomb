...
$(document).ready(function () {
    ...
    $('#xbtnSave').click(function () {
        ...
        localStorage.setItem("Hexcolsettings", JSON.stringify(settings));
    ...
    try {
        settings = JSON.parse(localStorage.getItem("Hexcolsettings"));
    }
    catch(err) {
        settings = null;
    }
    ...
    $("#bgcolor").spectrum("set", '#' + GColor.toHex(settings.bgcolor));
		...
		$('#bgshapes').val(json.bgshapes).selectmenu('refresh');
